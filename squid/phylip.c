 ...
 * format files.
 * 
 * CVS $Id: phylip.c,v 1.3 2004/03/11 17:57:37 jason Exp $
 */
...
ReadPhylip(MSAFILE *afp)
{
  ...
  int   nseq, alen;
  int   idx,idxseen;			/* index of current sequence */
  int   slen;
  int   nblock;
  ...
  while ((s = MSAFileGetLine(afp)) != NULL)
	...
	Die("nseq and/or alen not an integer in first line of PHYLIP file %s (%s)\n", afp->fname,s);
      nseq = atoi(s1);
      alen = atoi(s2);
      break;
    }
  if( nseq == 0 ) return NULL;
